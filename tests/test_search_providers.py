...
def test_get_search_secret(tmp_path):
    ...
    store = RuntimeSettingsStore(
        settings_path=tmp_path / "settings.json", secrets_path=tmp_path / "secrets.json"
    )
...
async def test_tavily_search_success():
    ...
    with (
        patch("silex.tools.search.get_search_secret", return_value="mock-tavily-key"),
        patch(
...
async def test_brave_search_success():
                ...
                "results": [
                    {
                        "title": "Brave Title",
                        "url": "https://brave.com",
                        "description": "Brave snippet description",
                    }
                ]
            }
        },
    ...
    with (
        patch("silex.tools.search.get_search_secret", side_effect=mock_secret_lookup),
        patch(
...
async def test_duckduckgo_fallback_when_apis_fail():
    ...
    with (
        patch("silex.tools.search.get_search_secret", return_value=""),
...
async def test_browser_fallback_when_ddg_fails():
    ...
    with (
        ...
        mock_browser.execute = AsyncMock(
            side_effect=[
        ...
        mock_browser.execute.assert_any_call(
            action="navigate", url="https://html.duckduckgo.com/html/?q=browser search"
        )
