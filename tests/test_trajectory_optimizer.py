...
from silex.evolution.trajectory_optimizer import TrajectoryRevisionResult, TrajectoryRefinementResult
...
async def test_trajectory_revision(tmp_path: Path):
    ...
    try:
        ...
        await db.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("traj_fail_1", 1, "view_file", "path/to/file", "some contents", "fact", 200.0, 100)
        )
        await db.execute(
            """
            INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("traj_fail_1", 2, "run_command", "pytest", "ModuleNotFoundError: No module named 'foo'", "hypothesis", 1500.0, 500)
        )
...
async def test_trajectory_recombination(tmp_path: Path):
    ...
    try:
        ...
        await db.execute(
            "INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("t1", 2, "view_file", "parser.py", "def parse(): pass", "fact", 200.0, 150)
        )
        await db.execute(
            "INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("t1", 3, "run_command", "pytest parser.py", "parser passed", "decision", 1000.0, 300)
        )
        ...
        await db.execute(
            "INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("t2", 2, "view_file", "parser.py", "def parse(): pass", "fact", 200.0, 150)
        )
        await db.execute(
            "INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("t2", 3, "replace_file_content", "parser.py", "modified ok", "decision", 500.0, 200)
        )
        await db.execute(
            "INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        new_traj = await db.fetch_one("SELECT * FROM trajectories WHERE trajectory_id = ?", (recomb_id,))
...
async def test_trajectory_refinement(tmp_path: Path):
    ...
    try:
        ...
        await db.execute(
            "INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("t_refine", 2, "run_command", "pytest", verbose_out, "decision", 1500.0, 800)
        )
        # Duplicate consecutive step
        await db.execute(
            "INSERT INTO trajectory_steps (trajectory_id, step_order, action_name, tool_input, execution_output, epistemic_category, latency_ms, token_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("t_refine", 3, "run_command", "pytest", verbose_out, "decision", 1500.0, 800)
        )
