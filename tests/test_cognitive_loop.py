...
def test_acquire_process_lock_cleans_up_stale_lock(tmp_path, monkeypatch, caplog):
    ...
    loop._process_lock_path.write_text(
        json.dumps({"pid": 424242, "role": "old"}), encoding="utf-8"
    )
    ...
    monkeypatch.setattr(
        "silex.core.cognitive_loop.get_process_role", lambda: "test-role"
    )
...
async def test_store_memories_marks_normative_and_character_provenance():
    ...
    loop.session = SimpleNamespace(
        current=SimpleNamespace(id="test-session", turn_count=0)
    )
...
async def test_background_memory_extraction_pipeline(tmp_path):
    ...
    from silex.models.schemas import (
        NewMemory as TestNewMemory,
        CausalObservation as TestCausalObservation,
    )
    from silex.core.cognitive_loop import ChatMemoryExtraction
    from unittest.mock import AsyncMock, patch, MagicMock

    with (
        patch("silex.utils.config.SILEX_VECTOR_DB", tmp_path / "vector_db"),
        ...
        try:
            ...
            with patch.object(
                loop.db, "transaction", wraps=loop.db.transaction
            ) as mock_tx:
