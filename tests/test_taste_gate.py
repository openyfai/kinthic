...
from silex.core.taste import TasteEvaluator, TasteFrictionBlock, TasteResponse, TasteScores

@pytest.mark.asyncio
async def test_taste_gate_accepts():
    mock_llm = AsyncMock()
    mock_llm.complete_json.return_value = TasteResponse(
        scores=TasteScores(simplicity=0.9, performance=0.9, robustness=0.9, security=0.9),
...
async def test_taste_gate_rejects_bloat():
    mock_llm = AsyncMock()
    mock_llm.complete_json.return_value = TasteResponse(
        scores=TasteScores(simplicity=0.4, performance=0.5, robustness=0.7, security=0.9),
    ...
    with pytest.raises(TasteFrictionBlock) as excinfo:
        await evaluator.evaluate("Setup Celery + Redis + Postgres to run a single task daily.")
