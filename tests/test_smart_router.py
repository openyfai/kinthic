...
def test_smart_router_init():
    ...
    with patch(
        "silex.utils.config.get_provider_settings",
        return_value={
            "provider": "gemini",
...
async def test_smart_router_fallback():
    ...
    with patch(
        ...
        with patch("silex.llm.factory.build_provider") as mock_build:
            ...
            with patch(
                "silex.utils.config.get_provider_secret",
                side_effect=lambda name, store: (
                    "fake_key" if name == "anthropic" else None
                ),
            ):
                with (
                    patch("silex.llm.registry.get_provider_profile") as mock_profile,
                    patch(
                        "silex.llm.registry.get_provider_client_class"
