...
from silex.evolution.admission_control import SkillAdmissionController, calculate_rouge_l
...
def test_rouge_l_similarity():
    ...
    sim = calculate_rouge_l("the quick brown fox jumps over the lazy dog", "quick brown fox jumps over cats")
    assert sim > 0.5
    assert sim < 1.0
...
async def test_novelty_computation(tmp_path: Path):
    ...
    try:
        ...
        assert await controller.compute_novelty_score("deploy to production") == pytest.approx(1.0)
        ...
        assert await controller.compute_novelty_score("deploy app to production container") == pytest.approx(0.0)
