...
def test_humanize_invalid_key() -> None:
    msg, hint, code = humanize_llm_error(Exception('Error 401: Incorrect API key provided'), "openai")
    assert code == "invalid_api_key"
    assert "dashboard" in hint.lower() or "key" in hint.lower()


def test_humanize_rate_limit() -> None:
    msg, hint, code = humanize_llm_error(Exception("429 Too Many Requests"), "openrouter")
