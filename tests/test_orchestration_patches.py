...
async def test_run_isolated_tool_validation(tmp_path):
    ...
    lease = ActuationLease.issue("task_1", "agent_1", allowed_tools=["custom_tool"])
...
async def test_docker_container_labels(tmp_path):
    ...
    lease = ActuationLease.issue(
        "task_1", "agent_1", allowed_tools=["run_terminal_command"]
    )

    with (
        patch("agent.compute.runtimes.docker.docker", mock_docker),
        patch(
            "agent.compute.runtimes.docker.docker.from_env", return_value=mock_client
        ),
    ):
