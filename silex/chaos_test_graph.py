...
async def run_chaos_test():
    ...
    print("[CHAOS TEST] Injecting 500 identical nodes to trigger duplicate collision...")
    ...
    if row_count == 1:
        print(f"[CHAOS TEST] SUCCESS. 500 nodes collapsed into exactly {row_count} row via UUID5 determinism.")
    else:
    ...
    res = await mem_store.amac.evaluate_admission(bloated_payload, "fact", "", mock_novelty)
    
    if len(res.get("sanitized_content", "")) <= 850:
        print("[CHAOS TEST] SUCCESS. Payload gating truncated the 5000-byte payload.")
    else:
        print(f"[CHAOS TEST] FAILED. Payload was not truncated properly. Length: {len(res.get('sanitized_content', ''))}")
        sys.exit(1)
