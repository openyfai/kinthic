...
async def test_dag():
    ...
    orchestrator.register_step(
        "A", lambda c: mock_action(c, 1.0), mock_compensate, depends_on=[]
    )
    orchestrator.register_step(
        "B", lambda c: mock_action(c, 1.0), mock_compensate, depends_on=[]
    )
    orchestrator.register_step(
        "C", lambda c: mock_action(c, 1.0), mock_compensate, depends_on=[]
    )
    orchestrator.register_step(
        "D", lambda c: mock_action(c, 0.1), mock_compensate, depends_on=["A", "B", "C"]
    )
    ...
    if duration < 2.0:
        print(
            f"[CHAOS TEST] DAG SUCCESS. Completed 3.1s of compute in {duration:.2f}s due to parallel execution."
        )
    else:
        print(
            f"[CHAOS TEST] DAG FAILED. Took {duration:.2f}s, which means it ran sequentially."
        )
        sys.exit(1)
...
async def test_entropy():
    ...
    if "old_node_id" not in ids and "new_node_id" in ids:
        print(
            "[CHAOS TEST] ENTROPY SUCCESS. Old node was hard-purged, new node survived."
        )
    else:
