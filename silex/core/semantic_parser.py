...
class SemanticParser:
    ...
    def __init__(
        self, ontology: Ontology, initial_profiles: dict[str, list[str]] = None
    ):
    ...
    def analyze_input(self, user_input: str, context: dict = None) -> dict:
        ...
        if "delete" in analysis["tokens"] and "identity" in normalized_input:
            analysis["causal_inferences"].append(
                "User is testing system identity/resilience."
            )
            analysis["potential_actions"].append("Explain architectural identity.")

        if (
            "privacy" in analysis["identified_concepts"]
            or "consent" in analysis["identified_concepts"]
        ) and any(
            token in analysis["tokens"]
            for token in ["read", "access", "open", "share", "send"]
        ):
            analysis["causal_inferences"].append(
                "The request may involve boundaries around consent, access, or confidentiality."
            )
            analysis["potential_actions"].append(
                "Clarify scope before acting on sensitive data."
            )

        if "autonomy" in analysis["identified_concepts"] and any(
            token in analysis["tokens"]
            for token in ["safe", "moral", "control", "aligned"]
        ):
            analysis["causal_inferences"].append(
                "The user is linking freedom of action with governance or alignment constraints."
            )
            analysis["potential_actions"].append(
                "Distinguish autonomy from unchecked capability."
            )
    ...
    def _extract_context_window(user_input: str, term: str, radius: int = 45):
        match = re.search(
            r"\b" + re.escape(term) + r"\b", user_input, flags=re.IGNORECASE
        )
        if not match:
