...
class LocalAlignmentVerifier:
    def __init__(self, baseline_prompt: str, stability_threshold: float = -0.015, variance_budget: float = 0.15):
        self.baseline_prompt = baseline_prompt
        self.stability_threshold = stability_threshold # Minimum allowable slope for beta regression
    ...
    def verify_mutation(self, mutated_prompt: str, locked_constraints: List[str]) -> Tuple[float, float]:
        """
        ...
        norm_product = np.linalg.norm(self.baseline_embedding) * np.linalg.norm(mutated_embedding)
        cosine_similarity = float(dot_product / norm_product) if norm_product != 0 else 0.0
        ...
        preservation_score = matched_constraints / len(locked_constraints) if locked_constraints else 1.0
        ...
        logger.info(f"Mutation evaluated. Cosine Drift: {cosine_drift:.4f}, Composite Score: {composite_score:.4f}")
    ...
    def analyze_drift_trend(self) -> bool:
        ...
        if self.cosine_drifts and self.cosine_drifts[-1] > self.variance_budget:
            logger.critical(f"Instant alignment halt: current drift {self.cosine_drifts[-1]:.4f} exceeds variance budget {self.variance_budget:.4f}.")
            return False
        ...
        logger.info(f"OLS analysis complete over {n} iterations. Trend slope (beta): {beta:.6f}")
        
        # A negative beta slope indicates progressive decay of constraint preservation
        if beta < self.stability_threshold:
            logger.critical(f"Systematic degradation detected: prompt slope of {beta:.6f} breaches stability thresholds.")
            return False
