...
class AutonomousSagaOrchestrator:
    ...
    def register_step(
        ...
        self.registered_steps.append(
            SagaStep(name, action, compensate, is_pivot, depends_on)
        )
        logger.info(
            f"Step '{name}' registered (Pivot: {is_pivot}, Depends: {depends_on})."
        )
    ...
    async def execute(self, context: SagaContext) -> bool:
        ...
        async def run_step(step: SagaStep):
            ...
            try:
                ...
                logger.error(
                    f"Unrecoverable execution fault at step '{step.name}': {str(ex)}"
                )

                if pivot_passed:
                    logger.critical(
                        f"Fault occurred post-pivot at step '{step.name}'. Rollback impossible. Escalating forward retry policies."
                    )
                    retry_success = await self._forward_retry(step, context)
                    if retry_success:
                        completed_steps.append(step)
                        context.execution_logs.append(
                            f"SUCCESS (RECOVERED): {step.name}"
                        )
                        await self._safe_telemetry_write(
                            "UPDATE saga_telemetry_logs SET current_step =?, status =? WHERE saga_id =?;",
                            (step.name, "STEP_COMPLETED", str(context.saga_id)),
    ...
    async def _forward_retry(self, step: SagaStep, context: SagaContext) -> bool:
        ...
        for attempt in range(1, max_retries + 1):
            logger.info(
                f"Attempting forward retry #{attempt} for post-pivot step '{step.name}' in {delay:.2f}s..."
            )
            await asyncio.sleep(delay)
            try:
                await step.action(context)
                logger.info(
                    f"Forward retry #{attempt} for step '{step.name}' succeeded!"
                )
                return True
            except Exception as ex:
                logger.warning(
                    f"Forward retry #{attempt} for step '{step.name}' failed: {ex}"
                )
                delay *= backoff_factor

        return False

    async def _rollback_compensable_steps(
        ...
        logger.warning(
            f"Initiating Saga rollback sequence. Compensations to execute: {len(completed_steps)}"
        )
        ...
        for step in reversed(completed_steps):
            logger.warning(
                f"Reverting step '{step.name}' via compensating action: {step.compensate.__name__}"
            )
            try:
                ...
                logger.critical(
                    f"FATAL: Compensating action for step '{step.name}' failed: {str(comp_ex)}"
                )
                await self._safe_telemetry_write(
                    "UPDATE saga_telemetry_logs SET status =? WHERE saga_id =?;",
    ...
    async def _escalate_post_pivot_failure(
        self, step: SagaStep, context: SagaContext, ex: Exception
    ):
        ...
        logger.critical(
            f"CRITICAL FAULT: Post-pivot failure at step '{step.name}' in Saga '{context.saga_id}'. Exception: {ex}"
        )
        await self._safe_telemetry_write(
            "UPDATE saga_telemetry_logs SET status =?, current_step =? WHERE saga_id =?;",
