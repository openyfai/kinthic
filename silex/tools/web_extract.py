...
class WebExtractTool(BaseTool):
    ...
    schema = {
        "url": "string (the full HTTP/HTTPS url to extract)"
    }
    ...
    async def execute(self, **kwargs) -> str:
        ...
        if not self.llm:
            log.warning("No LLM available for compression. Returning truncated raw text.")
        ...
        CHUNK_SIZE = 100_000
        chunks = [markdown[i:i+CHUNK_SIZE] for i in range(0, char_count, CHUNK_SIZE)]
