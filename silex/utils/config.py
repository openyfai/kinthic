...
def ensure_kinthic_home() -> None:
    ...
    KINTHIC_HOME.mkdir(exist_ok=True)
    (KINTHIC_HOME / "storage").mkdir(exist_ok=True)
    (KINTHIC_HOME / "config").mkdir(exist_ok=True)
    (KINTHIC_HOME / "workspace").mkdir(exist_ok=True)
    (KINTHIC_HOME / "runtime").mkdir(exist_ok=True)
    (KINTHIC_HOME / "logs").mkdir(exist_ok=True)
    KINTHIC_SKILLS.mkdir(exist_ok=True)
    ...
    if (KINTHIC_HOME / "memory").exists() and not any(
        (KINTHIC_HOME / "memory").iterdir()
    ):
        try:
    ...
    KINTHIC_SKILLS.mkdir(exist_ok=True)
    KINTHIC_LOGS.mkdir(exist_ok=True)
    KINTHIC_PLUGINS_TOOLS.mkdir(parents=True, exist_ok=True)
    KINTHIC_PLUGINS_SKILLS.mkdir(parents=True, exist_ok=True)
    KINTHIC_TRACES.mkdir(parents=True, exist_ok=True)
    KINTHIC_BACKUPS.mkdir(parents=True, exist_ok=True)
    KINTHIC_PLUGINS_PROVIDERS.mkdir(parents=True, exist_ok=True)

    # 5. Skills README + seed bundled skills from the package checkout
    readme_path = KINTHIC_SKILLS / "README.md"
    if not any(KINTHIC_SKILLS.iterdir()) or not readme_path.exists():
    ...
    if old_db.exists() and not SILEX_DB.exists():
        ...
        log.warning(
            "WARNING: Both old database (data/aria.db) and new database (~/.kinthic/storage/silex.db) exist. Using new database."
        )
    ...
    old_secrets = [old_data_dir / "secrets.json", KINTHIC_HOME / "secrets.json"]
    for os_sec in old_secrets:
        if os_sec.exists() and not KINTHIC_SECRETS.exists():
            shutil.copy2(os_sec, KINTHIC_SECRETS)
            log.info(f"Migrated secrets from {os_sec} to {KINTHIC_SECRETS}")
            break
...
def get_provider_settings(settings_store=None) -> dict:
    ...
    provider = (
        os.getenv("SILEX_PROVIDER")
        or os.getenv("ARIA_PROVIDER")
        or saved.get("provider", "gemini")
    )
    model = (
        os.getenv("SILEX_MODEL")
        or os.getenv("ARIA_MODEL")
        or saved.get("model", "gemini-3.1-flash-lite")
    )
...
def get_search_secret(provider_id: str, settings_store=None) -> str:
    ...
    # Env fallbacks
    env_map = {"tavily": "TAVILY_API_KEY", "brave": "BRAVE_API_KEY"}
...
def get_log_level() -> str:
    ...
    return (
        os.getenv("SILEX_LOG_LEVEL") or os.getenv("ARIA_LOG_LEVEL") or "INFO"
    ).upper()
...
def terminal_execution_enabled() -> bool:
    ...
    return env_flag("SILEX_ENABLE_TERMINAL_EXECUTION") or env_flag(
        "ARIA_ENABLE_TERMINAL_EXECUTION",
        _saved_security_flag("terminal_execution", False),
    )
...
def code_apply_enabled() -> bool:
    ...
    return env_flag("SILEX_ENABLE_CODE_APPLY") or env_flag(
        "ARIA_ENABLE_CODE_APPLY", _saved_security_flag("code_apply", False)
    )


def browser_actions_enabled() -> bool:
    """Whether Kinthic may use the browser automation tool."""
    return env_flag("SILEX_ENABLE_BROWSER_ACTIONS") or env_flag(
        "ARIA_ENABLE_BROWSER_ACTIONS", _saved_security_flag("browser_actions", True)
    )


def background_actions_enabled() -> bool:
    """Whether Kinthic may wake itself up to work on active goals."""
    return env_flag("SILEX_ENABLE_BACKGROUND_LOOP") or env_flag(
        "ARIA_ENABLE_BACKGROUND_LOOP", _saved_security_flag("background_actions", False)
    )


def require_tool_approvals() -> bool:
    """Whether high-risk tools should enter a pending approval queue."""
    return env_flag("SILEX_REQUIRE_TOOL_APPROVALS") or env_flag(
        "ARIA_REQUIRE_TOOL_APPROVALS",
        _saved_security_flag("require_tool_approvals", True),
    )
...
def telegram_public_mode_enabled() -> bool:
    settings_value = bool(
        _settings_store.load_settings().get("telegram", {}).get("public_mode", False)
    )
