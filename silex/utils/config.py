...
_workspace_env = os.getenv("KINTHIC_WORKSPACE") or os.getenv("SILEX_WORKSPACE") or os.getenv("KINTHIC_WORKSPACE") or os.getenv("ARIA_WORKSPACE")
if _workspace_env:
    WORKSPACE_DIR = Path(_workspace_env).resolve()
    if not str(WORKSPACE_DIR).startswith(str(KINTHIC_WORKSPACE)):
        logging.getLogger("kinthic.init").warning(f"SECURITY WARNING: Workspace directory resolved to {WORKSPACE_DIR} outside {KINTHIC_WORKSPACE}")
else:
...
def ensure_kinthic_home() -> None:
    ...
    if legacy_vyn_home.exists() and not (KINTHIC_HOME / "storage" / "silex.db").exists():
        try:
            ...
            for item in legacy_vyn_home.iterdir():
                if item.name == "vyn.db":
                    ...
                    dest_dir = KINTHIC_HOME / "config" if item.name == "plugins" else KINTHIC_HOME / "workspace" / item.name
                    shutil.copytree(item, dest_dir, dirs_exist_ok=True)
                else:
                    shutil.copy2(item, KINTHIC_HOME / "config" / item.name)
            log.info("✓ Successfully migrated existing profile from ~/.vyn to new ~/.kinthic layout")
    ...
    safe_migrate_file(KINTHIC_HOME / "silex.db-wal", KINTHIC_HOME / "storage" / "silex.db-wal")
    safe_migrate_file(KINTHIC_HOME / "silex.db-shm", KINTHIC_HOME / "storage" / "silex.db-shm")
    ...
    if (KINTHIC_HOME / "memory").exists() and not any((KINTHIC_HOME / "memory").iterdir()):
        try:
    ...
    if old_db.exists() and not SILEX_DB.exists():
        ...
        log.warning("WARNING: Both old database (data/aria.db) and new database (~/.kinthic/storage/silex.db) exist. Using new database.")
    ...
    if not KINTHIC_PERSONA.exists():
        ...
        try:
            ...
            log.info("Initialized default persona.yaml at ~/.kinthic/config/persona.yaml")
        except Exception as e:
