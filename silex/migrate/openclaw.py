...
def scan_openclaw(source_path: str | None = None) -> dict[str, Any]:
    ...
    base_dir = (
        Path(source_path).expanduser() if source_path else Path.home() / ".openclaw"
    )
    ...
    if workspace_dir.exists() and workspace_dir.is_dir():
        ...
        skills = [
            ...
            if f.name not in ["AGENTS.md", "SOUL.md", "MEMORY.md"]
        ]
...
def import_openclaw(source_path: str | None = None, dry_run: bool = True) -> list[str]:
    ...
    if config_file.exists():
        logs.append(
            "📄 Found openclaw.json, please manually review the allowlists for Kinthic."
        )
    ...
    if workspace_dir.exists() and workspace_dir.is_dir():
        ...
        if (workspace_dir / "SOUL.md").exists():
            logs.append(
                "👤 Found SOUL.md identity file, please manually review it for Kinthic personas."
            )

    logs.append("✅ Migration complete.")
    logs.append(
        "🔒 IMPORTANT: You must run `kinthic telegram` and /pair your account to ensure security boundaries are established."
    )
    return logs
