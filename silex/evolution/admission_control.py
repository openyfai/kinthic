...
class SkillAdmissionController:
    ...
    async def compute_novelty_score(self, content: str) -> float:
        ...
        """
        active_skills = list(self.skills_dir.glob("*.md")) + list(
        ...
        for skill_path in active_skills:
            try:
                ...
                log.warning(
                    f"Failed to read existing skill {skill_path} for similarity comparison: {e}"
                )

        # Novelty is inverse of max overlap
        novelty = 1.0 - max_similarity
        log.debug(
            f"Novelty check complete. Max similarity overlap: {max_similarity:.4f}, Novelty: {novelty:.4f}"
        )
        return novelty
    ...
    async def admit_skill(
        ...
        log.info(
            f"Skill '{skill_name}' evaluated with A-MAC score: {composite_score:.4f} (threshold={threshold:.2f})"
        )
        if composite_score < threshold:
            log.warning(
                f"Skill '{skill_name}' rejected due to insufficient A-MAC quality score."
            )
        ...
        memory_id = (
            f"mem_skill_{hashlib.md5(skill_name.encode('utf-8')).hexdigest()[:12]}"
        )
        integrity_hash = hashlib.sha256(
            full_markdown_payload.encode("utf-8")
        ).hexdigest()
