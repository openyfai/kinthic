...
class StepRevision(BaseModel):
    ...
    revised_category: str = Field(
        ..., description="Must be one of 'decision', 'hypothesis', 'fact', 'dead_end'"
    )
    reasoning: str = Field(
        ..., description="Detailed explanation for this category revision"
    )
...
class TrajectoryRevisionResult(BaseModel):
    critique: str = Field(
        ...,
        description="Reflecting critique on the trajectory failure and error patterns",
    )
    step_revisions: list[StepRevision] = Field(
        ..., description="List of step category adjustments"
    )


class StepRefinement(BaseModel):
    step_order: int = Field(..., description="The step order index (1-based) to refine")
    compressed_output: str = Field(
        ..., description="Summarized/compressed output maintaining critical debug info"
    )
    token_saved_estimate: int = Field(
        ..., description="Estimated tokens saved by compressing this output"
    )


class TrajectoryRefinementResult(BaseModel):
    rationale: str = Field(
        ..., description="Explanation of why certain steps were compressed"
    )
    step_refinements: list[StepRefinement] = Field(
        ..., description="List of step payload compressions"
    )
...
class TrajectoryOptimizer:
            ...
            steps_summary.append(
                {
                    ...
                    "execution_output": s["execution_output"][:300]
                    + ("..." if len(s["execution_output"]) > 300 else ""),
        ...
        async with self.db.transaction():
            for rev in revision_data.step_revisions:
                if rev.revised_category in (
                    ...
                    log.debug(
                        f"Mutated step {rev.step_order} to category {rev.revised_category}"
                    )
    ...
    async def run_recombination(
        self, trajectory_id_1: str, trajectory_id_2: str
    ) -> str | None:
        """
        ...
        """
        log.info(
            f"Running Recombination between {trajectory_id_1} and {trajectory_id_2}"
        )
        ...
        if not t1["is_success"] or not t2["is_success"]:
            log.warning(
                "Recombination expects successful trajectories as parent paths."
            )
            return None
        ...
        for i, s1 in enumerate(steps1):
            for j, s2 in enumerate(steps2):
                if (
                    s1["action_name"] == s2["action_name"]
                    and s1["action_name"] != "initialize"
                ):
        ...
        log.info(
            f"Crossover point identified at action '{action}' (T1 index {idx1}, T2 index {idx2})"
        )
    ...
    async def run_refinement(self, trajectory_id: str) -> str:
        ...
        for s in steps:
            if pruned_steps:
                last = pruned_steps[-1]
                if (
                    last["action_name"] == s["action_name"]
                    and last["tool_input"] == s["tool_input"]
                ):
                    log.info(
                        f"Pruning duplicate consecutive call to tool: {s['action_name']}"
                    )
                    continue
            pruned_steps.append(
                {
        ...
        if heavy_steps:
            ...
            heavy_summary = [
                {"step_order": s["step_order"], "output": s["execution_output"]}
                for s in heavy_steps
            ]
            user_input = json.dumps({"steps_to_compress": heavy_summary}, indent=2)

            try:
                refinement_data: TrajectoryRefinementResult = (
                ...
                refinement_map = {
                    ref.step_order: ref for ref in refinement_data.step_refinements
                }
                for s in pruned_steps:
                    if s["step_order"] in refinement_map:
                        ...
                        s["token_usage"] = max(
                            50, s["token_usage"] - ref.token_saved_estimate
                        )
                        log.debug(
                            f"Compressed output for step {s['step_order']}, saved ~{ref.token_saved_estimate} tokens."
                        )
            except Exception as e:
                log.warning(
                    f"LLM footprint compression failed, falling back to heuristic: {e}"
                )
