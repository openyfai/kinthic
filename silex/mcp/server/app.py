...
from silex.mcp.server.lifecycle import McpServerContext
...
def register_tools(mcp: "FastMCP", ctx: McpServerContext) -> None:
    ...
    @mcp.tool()
    async def kinthic_skills_list() -> str:
        """List loaded Kinthic workflow skills (compact index)."""
    ...
    @mcp.tool()
    async def kinthic_skill_view(name: str) -> str:
        """Load full markdown instructions for a named Kinthic skill."""
        loader = ctx.skill_loader
        if loader is None:
            from silex.core.skills import SkillLoader
            loader = SkillLoader()
        ...
        if body is None:
            known = ", ".join(sorted(loader.skills.keys())[:20])
            return f"Skill '{name}' not found. Known: {known}"
        meta = loader.skill_meta.get(name.strip())
        header = f"# Skill: {name}\n"
...
def build_mcp_http(ctx: McpServerContext) -> "FastMCP":
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "Use kinthic_skills_list and kinthic_skill_view for workflow skills."
        ),
...
def mount_mcp_server(app: "FastAPI", cognitive_loop) -> "FastMCP":
    ...
    """
    from silex.mcp.server.lifecycle import context_from_loop
