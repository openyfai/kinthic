...
def install_service(*, force: bool = False) -> tuple[bool, str]:
    ...
    if not exec_path.exists() and shutil.which(str(exec_path)) is None:
        return (
            False,
            f"kinthic executable not found at {exec_path}. Run the installer first.",
        )
    ...
    if system == "Linux":
        if not shutil.which("systemctl"):
            return (
                False,
                "systemctl not found. Install systemd or run 'kinthic daemon run' in foreground.",
            )
        ...
        if unit_path.exists() and not force:
            return (
                False,
                f"Service already installed at {unit_path}. Use --force to overwrite.",
            )
    ...
    if system == "Darwin":
        ...
        if plist_path.exists() and not force:
            return (
                False,
                f"LaunchAgent already installed at {plist_path}. Use --force to overwrite.",
            )
