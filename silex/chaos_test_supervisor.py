...
async def main():
    ...
    mock_decision = EthicalDecision(action=EthicalAction.PROCEED, principle="test", rationale="test", risk_level="read_only", requires_consent=False, uncertainty=0.0, context="test")
    registry.ethics.evaluate_tool_call = unittest.mock.MagicMock(return_value=mock_decision)
    registry._log_ethical_decision = unittest.mock.AsyncMock()

    call = ToolCall(tool_name="hanging_tool", arguments="{}", expected_outcome="Nothing", rationale="For testing")
    ...
    print(f"-> Registry returned result: success={result.success}, outcome='{result.actual_outcome}'")
    ...
    if "timed out" not in result.actual_outcome.lower():
        print(f"[CHAOS TEST] FAILED. The tool failed, but not because of a timeout. Outcome: {result.actual_outcome}")
        sys.exit(1)
        
    print("[CHAOS TEST] SUCCESS. The Global Execution Supervisor successfully detected the infinite hang, severed the thread, and returned a structured timeout failure without deadlocking the engine.")
