...
async def main():
    ...
    for i in range(3):
        try:
            ...
            if isinstance(e, CircuitBreakerTripped):
                ...
                print(
                    f"-> Attempt {i + 1} raised generic exception (Retries exhausted)."
                )
    ...
    if duration > 0.05:
        print(
            f"[CHAOS TEST] FAILED. Fast-fail took too long: {duration}s. Network is still blocking."
        )
        sys.exit(1)
    ...
    if cb.state != CircuitBreakerState.CLOSED:
        print(
            f"[CHAOS TEST] FAILED. Circuit Breaker did not RESET to CLOSED. State is {cb.state}"
        )
        sys.exit(1)

    print(
        "[CHAOS TEST] SUCCESS. Circuit Breaker successfully tripped, fast-failed, cooled down, and reset."
    )
