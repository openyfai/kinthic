"""
Terminal UI — ARIA's Rich-based interface.
...
from silex.models.schemas import CognitiveResponse, Goal, Memory, Session
...
def show_startup_summary(
    ...
    if memory_count == 0 and session_count <= 1:
        ...
        console.print(
            ...
            f"[aria.goal]{goal_count} active goals[/][dim],[/] "
            f"[aria.stat]{total_turns} total turns[/] [dim]across[/] "
            f"[aria.stat]{session_count} sessions[/][dim].[/]"
        )
        console.print(
            "  [dim]  Type your message or[/] [aria.command]:help[/] [dim]for commands.[/]\n"
        )
...
def show_response(cognitive: CognitiveResponse) -> None:
    ...
    if reflection and len(reflection) > 10:
        monologue.append("\n\n[🪞 Reflection]\n", style="bold magenta")
        monologue.append(reflection, style="aria.reflection")
    ...
    if conf >= 0.7:
        conf_style = "aria.confidence.high"
        conf_icon = "●"
    elif conf >= 0.4:
        conf_style = "aria.confidence.mid"
        conf_icon = "◐"
    else:
        conf_style = "aria.confidence.low"
        conf_icon = "○"
    ...
    status_parts = [
        ...
        f"[aria.goal]{len(cognitive.goal_updates)} goals[/]",
    ]
    ...
    if cognitive.uncertainty_flags:
        ...
        status_parts.append(f"[aria.dim]⚠ {flags}[/]")
...
def show_memories(memories: list[Memory]) -> None:
    ...
    if not memories:
        console.print("\n  [aria.dim]No memories stored yet.[/]\n")
        return
...
def show_search_results(memories: list[Memory], query: str) -> None:
    ...
    if not memories:
        console.print(f"\n  [aria.dim]No memories matching '{query}'.[/]\n")
        return
...
[bold bright_white]🧠 Memory[/]
  ...
  [aria.command]:search <query>[/]    Search memories by keyword
  [aria.command]:remember <fact>[/]   Manually store a memory
  [aria.command]:forget <#>[/]        Delete a memory by its number

[bold bright_white]🌐 World Model[/]  [dim](Phase 2)[/]
  [aria.command]:graph[/]             Show knowledge graph statistics
  [aria.command]:graph <concept>[/]   Show neighborhood of a concept
...
[bold bright_white]📈 Self-Improvement[/]  [dim](Phase 3)[/]
  [aria.command]:improvements[/]      Show recent self-corrections

[bold bright_white]⚖️ Debate Engine[/]  [dim](Phase 4)[/]
  [aria.command]:debate <topic>[/]     Force A/B debate on a topic
  [aria.command]:uncertainties[/]     Show known knowledge gaps

[bold bright_white]🛠️ Tool Use[/]  [dim](Phase 5)[/]
  [aria.command]:tools[/]             Show registered tools

[bold bright_white]📐 Generalization[/]  [dim](Phase 6)[/]
  [aria.command]:principles[/]        Show discovered universal laws
...
[bold bright_white]📊 Info[/]
  [aria.command]:stats[/]             Show session statistics
  [aria.command]:sessions[/]          Show past session history

[bold bright_white]🤖 Routing[/]
  [aria.command]/providers[/]        List all providers and API key status
  [aria.command]/model <name>[/]     Switch the active LLM provider
  [aria.command]/mode <mode>[/]      Set routing mode (speed|quality|auto|local)

[bold bright_white]🔌 Plugins[/]  [dim](Phase D)[/]
  [aria.command]/plugins[/]           List all registered tools (built-in + user plugins)
  [aria.command]/plugin reload[/]     Hot-reload user plugins without restarting

[bold bright_white]📂 File RAG[/]  [dim](Phase C)[/]
  [aria.command]/index [path][/]      Index a folder into the vector DB
  [aria.command]/rag <query>[/]       Search the file index directly

[bold bright_white]🎤 Voice I/O[/]  [dim](Phase E)[/]
  [aria.command]/voice [on|off][/]    Toggle voice input/output mode
...
def show_graph_neighborhood(data: dict) -> None:
    ...
    if not data or not data.get("center"):
        console.print("\n  [aria.dim]Concept not found in the graph.[/]\n")
        return
    ...
    console.print(tree)
    console.print(
        f"  [aria.dim]{len(data.get('nodes', []))} nodes, "
...
def show_causal_chain(chain: list[dict], from_concept: str, to_concept: str) -> None:
    ...
    if not chain:
        console.print(
            f"\n  [aria.dim]No causal path found from "
...
def show_contradictions(contradictions) -> None:
    ...
    if not contradictions:
        console.print("\n  [aria.dim]No unresolved contradictions.[/]\n")
        return
...
def show_hypotheses(hypotheses) -> None:
    """Display hypotheses."""
    if not hypotheses:
        console.print("\n  [aria.dim]No pending hypotheses.[/]\n")
        return
...
def show_improvements(improvements) -> None:
    ...
    if not improvements:
        console.print("\n  [aria.dim]No self-improvements logged yet.[/]\n")
        return
...
def show_uncertainties(uncertainties) -> None:
    ...
    if not uncertainties:
        console.print("\n  [aria.dim]No known uncertainties tracked.[/]\n")
        return
...
def show_proposals(proposals) -> None:
    ...
    if not proposals:
        console.print("\n  [aria.dim]No pending self-improvement proposals.[/]\n")
        return
