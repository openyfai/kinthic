...
class CausalObservation(BaseModel):
    ...
    evidence: str = Field(description="Why ARIA believes this relationship exists")
    strength: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
...
class EthicalDecision(BaseModel):
    ...
    risk_level: ToolRisk = Field(
        description="Risk class considered during the decision"
    )
...
class DebateArgument(BaseModel):
    ...
    evidence_or_logic: str = Field(
        description="Data, facts, or logical principles backing the claim"
    )
...
class DebateResolution(BaseModel):
    ...
    strongest_points_a: list[str] = Field(
        description="The most valid points made by Agent A"
    )
    strongest_points_b: list[str] = Field(
        description="The most valid points made by Agent B"
    )
    synthesis: str = Field(
        description="The final synthesized truth derived from the clash"
    )
...
class ActionLogEntry(BaseModel):
    ...
    model_update: str = Field(
        description="How ARIA updated her world model based on the result"
    )
    created_at: str = Field(
        default_factory=lambda: datetime.now(timezone.utc).isoformat()
    )
...
class UniversalPrinciple(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str = Field(
        description="A memorable name for the principle (e.g. 'The Friction Law')"
    )
    statement: str = Field(description="The abstract, domain-agnostic rule")
    original_domain: str = Field(
        description="The specific domain where this was first learned"
    )
...
class PrincipleExtractionResponse(BaseModel):
    ...
    has_principle: bool = Field(
        description="Whether a universal principle can be extracted"
    )
    ...
    applicable_domains: list[str] = Field(
        default_factory=list, description="Other domains where this applies"
    )
