...
class Memory(BaseModel):
    ...
    def generate_deterministic_id(self) -> "Memory":
        if not self.id:
            self.id = str(uuid.uuid5(SILEX_NAMESPACE, f"memory:{self.content.strip().lower()}"))
        return self
...
class KnowledgeNode(BaseModel):
    ...
    def generate_deterministic_id(self) -> "KnowledgeNode":
        if not self.id:
            ...
            self.id = str(uuid.uuid5(SILEX_NAMESPACE, f"node:{node_type_val}:{self.content.strip().lower()}"))
        return self
...
class CausalEdge(BaseModel):
    ...
    def generate_deterministic_id(self) -> "CausalEdge":
        if not self.id:
            edge_type_val = self.edge_type.value if hasattr(self.edge_type, 'value') else str(self.edge_type)
            self.id = str(uuid.uuid5(SILEX_NAMESPACE, f"edge:{self.source_node}:{edge_type_val}:{self.target_node}"))
        return self
