"""
silex/plugins/loader.py — Discovers and loads user tool plugins.

Unified plugin.yaml spec (all fields are optional except `name`):

  name: my_tool           # unique identifier (required)
  type: tool              # tool | skill | provider  (default: tool)
  description: "..."
  version: "1.0.0"
  author: "Your Name"
  tags: [tag1, tag2]
...
Example tool.py:
  ...
  class MyTool(BaseTool):
      ...
      requires_approval = False
      schema = {
          "query": {"type": "string", "description": "Input to process"},
      }
...
def load_tool_plugins(plugins_dir: Path) -> list["BaseTool"]:
    ...
    """
    from silex.tools.base import BaseTool  # local import to avoid circular deps
