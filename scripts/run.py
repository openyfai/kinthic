...
def _fmt_help() -> str:
    lines = [
        ...
        "  /debate <topic>  Run A/B debate on a topic",
...
def _fmt_debate_resolution(res) -> str:
    if not res:
        return "Debate produced no result."
    if isinstance(res, dict):
        lines = [
            f"Debate: {res.get('topic', '?')}",
            "",
            f"Position A: {res.get('position_a', '?')[:80]}",
            f"Position B: {res.get('position_b', '?')[:80]}",
            "",
        ...
        if reasoning:
            lines += ["", f"Reasoning: {reasoning[:200]}"]
        return "\n".join(lines)
    return str(res)[:400]
                ...
                # ── MCP ─────────────────────────────────────────────────────
                    ...
                    try:
                        ...
                        if mcp_action == "list":
                            ...
                            count = loop.tool_registry.reload_mcp_tools()
