# Kinthic Support Playbook

Having issues with Kinthic? Before opening an issue in Discord or GitHub, please review this playbook for common solutions.
...
3. Run `kinthic telegram run` again to generate a new pairing PIN.
4. Send only the exact PIN to the bot.

## Issue 2: Tool Approval Timeout / Hanging
**Symptom:** You are waiting for Kinthic to reply, but it seems to have frozen after you asked it to write a file or run a command.
...
**Solution:**
1. Run `kinthic usage` in the terminal (or `/usage` in Telegram) to see a breakdown of costs per model.
